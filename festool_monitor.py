...
for sale, diffs that against the last saved snapshot, and emails you when:
...
Pure standard library -- no `pip install` needed. Designed to run on a schedule
(e.g. GitHub Actions) where state.json is committed back to the repo between runs.

Local testing:
    python3 festool_monitor.py --dry-run      # scrape + print, no email, no state write
    python3 festool_monitor.py --test-email   # send one test email (needs SMTP env vars)
"""
...
USER_AGENT = "Mozilla/5.0 (compatible; festool-recon-monitor/1.0; +github-actions)"
...
def require_email_env(env):
    missing = [k for k in ("SMTP_USER", "SMTP_PASS", "MAIL_TO") if not env.get(k)]
    if missing:
        raise SystemExit(
            "Missing required email secrets: " + ", ".join(missing) +
...
def deliver(args, env, subject, body):
    if args.dry_run:
        log("\n=== DRY RUN — would send this email ===")
        log("Subject:", subject)
        log(body)
        log("=== end dry run ===\n")
        return
    require_email_env(env)
...
def build_baseline_body(current, watchlist):
    ...
    for item in sorted(current.values(), key=lambda x: x["title"].lower()):
        ...
        lines.append(fmt_item(item, bullet="🔥" if hits else "•"))
    lines.append("\nFrom now on you'll get an alert when something NEW comes on sale")
    lines.append("(🔥 = matches your watchlist).")
    lines.append(f"Watchlist: {', '.join(watchlist) if watchlist else '(empty — edit config.json)'}")
    lines.append(f"\nBrowse: {BROWSE_URL}")
    return "\n".join(lines)
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true",
                        help="scrape and print; do not send email or write state")
    ...
    if args.test_email:
        require_email_env(env)
        send_email(env, "✅ Festool Recon monitor — test",
                   "If you're reading this on your phone, your email/SMS setup works. 🎉")
        log("test email sent")
        return 0
    ...
    # Don't treat it as "everything sold out" and don't wipe our baseline.
    ...
    if not initialized:
        subject = f"🔔 Festool Recon monitor started — {len(current)} on sale now"
        deliver(args, env, subject, build_baseline_body(current, watchlist))
    ...
    for h in new_handles:
        item = current[h]
        hits = watchlist_hits(item["title"], watchlist)
        (urgent if hits else normal).append(item)

    sections, subject_bits, notify = [], [], False

    if urgent:
        notify = True
        subject_bits.append(f"🔥 {len(urgent)} WATCHED")
        sections.append("🔥 ON YOUR WATCHLIST — ON SALE NOW:\n" +
                        "\n".join(fmt_item(i, bullet="🔥") for i in urgent))

    if alert_on_new and normal:
        notify = True
        subject_bits.append(f"{len(normal)} new")
    ...
    if alert_on_sold_out and gone_handles:
        ...
        subject_bits.append(f"{len(gone)} sold out")
    ...
    subject = " • ".join(subject_bits) + " — Festool Recon"
    body = "\n\n".join(sections)
    body += f"\n\n{len(current)} item(s) on sale total.\nBrowse: {BROWSE_URL}"

    deliver(args, env, subject, body)  # raises if email fails -> state NOT advanced -> retried next run
    write_state(args, current)
    return 0
